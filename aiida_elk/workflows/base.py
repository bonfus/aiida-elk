# -*- coding: utf-8 -*-
"""Workchain to run a Elk calculation with automated error handling and restarts."""
...
class ElkBaseWorkChain(BaseRestartWorkChain):
    """Workchain to run a Elk calculation with automated error handling and restarts."""

    _process_class = ElkCalculation  # type: ignore[assignment]

    @classmethod
    def define(cls, spec):
        """Define the process specification."""
        super().define(spec)
        spec.expose_inputs(ElkCalculation, namespace='elk')
        spec.expose_outputs(ElkCalculation)
        spec.outline(
            cls.setup,
        ...
        spec.exit_code(
            310,
            'ERROR_NO_STATE_TO_RESTART',
            message='The calculation failed and did not retrieve a STATE.OUT file from which can be restarted.',
        )
    ...
    def setup(self):
        ...
        self.ctx.inputs = AttributeDict(self.exposed_inputs(ElkCalculation, 'elk'))  # type: ignore[arg-type]
    ...
    @process_handler(
        priority=500,
        exit_codes=[
            ElkCalculation.exit_codes.ERROR_IONIC_CONVERGENCE_NOT_REACHED,  # type: ignore[union-attr]
        ],
    )
    ...
    @process_handler(
        priority=410,
        exit_codes=[
            ElkCalculation.exit_codes.ERROR_ELECTRONIC_CONVERGENCE_NOT_REACHED,  # type: ignore[union-attr]
        ],
    )
    ...
    @process_handler(
        priority=110,
        exit_codes=[
            ElkCalculation.exit_codes.ERROR_SCHEDULER_NODE_FAILURE,  # type: ignore[union-attr]
        ],
    )
    ...
    @process_handler(
        priority=100,
        exit_codes=[
            ElkCalculation.exit_codes.ERROR_SCHEDULER_OUT_OF_WALLTIME,  # type: ignore[union-attr]
        ],
    )
    def handle_out_of_walltime(self, node):
        ...
        """
        if 'STATE.OUT' not in node.outputs:
            self.report_error_handled(node, 'aborting because the failed calculation does not provide a STATE.OUT.')
